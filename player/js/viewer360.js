 ...
 function doReconnect(){
  ...
  ///Need to seperate opus to tracks?
  ...
  // { type:'video/mp4', src:'video' },	
   ...
   let SETUP_CONFIG = {
      ...
      autoplay: true, 
            ...
            //playbackRates:[1, .5, .2], //for vod
        
		    src: SRC_CONFIG
		};
    
    let myPlayer = videojs('player',SETUP_CONFIG, function(){
      //Set up the 360 live view
...
/*
 ...
 let seekTime = videojs.currentTime;
 if (isNaN(seekTime)) { seekTime = 0.0; }
//Video JS is set up\

//Need multiple Audio Tracks
let audiotracksrc1 = "test.mp3";
//let audiotracksrc2 = audio[2];
...
(async () => {
  videojs.srcObject = await navigator.mediaDevices.getUserMedia({player: true});
  while (true) {
